...
from fastapi import FastAPI, UploadFile, File, HTTPException, Request # <--- Tambahan Request
from fastapi.responses import RedirectResponse # <--- Tambahan untuk redirect
...
# FUNGSI HELPER: Ekstraksi ML (Murni Inference)
def run_ml_ocr(image):
    # Mulai catat waktu INFERENCE murni
    ...
    for (bbox, text, prob) in results:
        ...
        if cropped_image.size > 0:
            pred = predict_crop_image(cropped_image)
            splitify_raw_list.append(pred)

    # Hitung total waktu inference
    inference_time = time.time() - start_inference
    
    # Simpan ke file log khusus inference
...
# ENDPOINT: Menampilkan Persentil Inference
@app.get("/lihat-log")
def kalkulasi_log_performa():
    try:
        # Buka dan baca file log inference
        with open("inference_latency.log", "r") as f:
            lines = f.readlines()
        
        # Ubah teks log jadi list angka (float)
        ...
        return {
            "status": "success",
            "total_inference_tercatat": len(latencies),
            "P50_Median": f"{p50:.4f} detik",
            "P90": f"{p90:.4f} detik",
            "P95": f"{p95:.4f} detik",
            "Paling_Cepat": f"{min(latencies):.4f} detik",
            "Paling_Lama": f"{max(latencies):.4f} detik"
        }
    except FileNotFoundError:
        return {"message": "File inference_latency.log belum dibuat. Coba test OCR sekali dulu."}

# ENDPOINT: Khusus untuk test performa ML (Tanpa LLM Gemini)
@app.post("/scan-ml")
...
# ENDPOINT UTAMA: Full pipeline (ML + Gemini)
@app.post("/scan")
def scan_receipt_full(file: UploadFile = File(...)):
    ...
    # 1. Ekstraksi teks menggunakan fungsi helper ML
    full_text = run_ml_ocr(image)

    # 2. Strukturisasi JSON menggunakan Gemini
    structure_template = """
    {
        "items": [
            {
...
if __name__ == "__main__":
    uvicorn.run("main:app", host="0.0.0.0", port=7860, workers=16)